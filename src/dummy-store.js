export default {
  "marketNames": [
    `-`,
    `Baldwin Hills Crenshaw Farmers Market`,
    `Mar Vista Farmer's Market`,
    `Culver City Farmer's Market`,
    `Boyle Heights Farmer's Market`,
    `Silver Lake Saturday Market`
  ],

  "vendorCategories": [
    'Fruits & Vegetables',
    'Grains & Nuts',
    'Poultry',
    'Meats',
    'Seafood',
    'Other'
  ],

  "vendorNames": [
    'At Vero Eos Et Accusamus | H-12',
    'Et Iusto Odio | H-12',
    'Dignissimos Ducimus | H-12',
    'Qui Blanditiis | H-12',
    'Praesentium Voluptatum | H-12',
    'Deleniti Atque | H-12',
    'Corrupti Quos Dolores | H-12',
    'Et Quas Molestias | H-12',
    'Excepturi | H-12',
    'Sint Occaecati | H-12',
    'Cupiditate Non Provident | H-12',
    'Similique Sunt | H-12'
  ],

  'itemsListing': [
    {
      'marketName': `Baldwin Hills Crenshaw Farmers Market`,
      'marketId': 123,
      'vendorName': `Fresh Greens Daily`,
      'vendorStall': 'H-34',
      'foods': {
        'chard': `0.99/lb`,
        'peaches': `1.99/lb`,
        'bananas': `2.99/bunch`
      }
    },
    {
      'marketName': `Boyle Heights Community Market`,
      'marketId': 456,
      'vendorName': `Lydia's Produce`,
      'vendorStall': '1192',
      'foods': {
        'pears': `3.99/lb`,
        'oranges': `4.99/lb`,
        'bananas': `5.99/lb`
      }
    },
    {
      'marketName': `Mar Vista Farmer's Market`,
      'marketId': 789,
      'vendorName': `Jenson Farms`,
      'vendorStall': '24C',
      'foods': {
        'apples': `6.99/lb`,
        'oranges': `7.99/lb`,
        'bananas': `8.49/bunch`
      }
    }
  ],

  "markets": [
    {
      'id': 123,
      'title': `Baldwin Hills Crenshaw Farmers Market`,
      'hero_image': `https://images.unsplash.com/photo-1498579397066-22750a3cb424?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=2850&q=80`,
      'schedule': `Saturdays from 9 a.m. to 5 p.m.`,
      'address': `3650 W. Martin Luther King Jr. Blvd.`,
      'summary': `Quia consequuntur magni dolores eos qui ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.`,
      'description': `The Baldwin Hills Crenshaw Farmers Market has offered locally-grown fruits and vegetables, and freshly-prepared foods for over 25 years. Here is some more information sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.****Here's even more information ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui.`,
      'categories': [`Fruits`, `Vegetables`, `Bakery`],
      'vendor_list': [
        { 'id': 110, 'title': `At vero eos et accusamus`, 'stall': `H-1` },
        { 'id': 111, 'title': `Et iusto odio`, 'stall': `H-2` },
        { 'id': 112, 'title': `Dignissimos ducimus`, 'stall': `H-3` },
        { 'id': 113, 'title': `Qui blanditiis`, 'stall': `H-4` },
        { 'id': 114, 'title': `Praesentium voluptatum`, 'stall': `H-5` },
        { 'id': 115, 'title': `Deleniti atque`, 'stall': `H-6` },
        { 'id': 116, 'title': `Corrupti quos dolores`, 'stall': `H-7` },
        { 'id': 117, 'title': `Et quas molestias`, 'stall': `H-8` },
      ]
    },
    {
      'id': 456,
      'title': `Boyle Heights Community Market`,
      'hero_image': `https://images.unsplash.com/photo-1498579397066-22750a3cb424?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=2850&q=80`,
      'schedule': `Sundays from 3:30 to 9 p.m.`,
      'address': `1831 E. 1st St.`,
      'summary': `Voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui.`,
      'description': `The Boyle Heights Community Market has offered locally-grown fruits and vegetables, and freshly-prepared foods for over 25 years. Here is some more information sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.****Here's even more information ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui.`,
      'categories': [`Vegetables`, `Bakery`, `Live Music`],
      'vendor_list': [
        { 'id': 210, 'title': `Dignissimos ducimus`, 'stall': `H-3` },
        { 'id': 211, 'title': `Qui blanditiis`, 'stall': `H-4` },
        { 'id': 212, 'title': `Praesentium voluptatum`, 'stall': `H-5` },
        { 'id': 213, 'title': `Deleniti atque`, 'stall': `H-6` },
        { 'id': 214, 'title': `Corrupti quos dolores`, 'stall': `H-7` },
        { 'id': 215, 'title': `Et quas molestias`, 'stall': `H-8` },
        { 'id': 216, 'title': `Excepturi`, 'stall': `H-9` },
        { 'id': 217, 'title': `Sint occaecati`, 'stall': `H-10` },
      ]
    },
    {
      'id': 789,
      'title': `Mar Vista Farmer's Market`,
      'hero_image': `https://images.unsplash.com/photo-1498579397066-22750a3cb424?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=2850&q=80`,
      'schedule': `Sundays from 9 a.m. to 2 p.m.`,
      'address': `3826 Grand View Blvd`,
      'summary': `Ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas.`,
      'description': `The Mar Vista Farmer's Market has offered locally-grown fruits and vegetables, and freshly-prepared foods for over 25 years. Here is some more information sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.****Here's even more information ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui.`,
      'categories': [`Fruits`, `Vegetables`, `Bakery`],
      'vendor_list': [
        { 'id': 320, 'title': `Similique sunt`, 'stall': `H-12` },
        { 'id': 321, 'title': `Cupiditate non provident`, 'stall': `H-11` },
        { 'id': 312, 'title': `Sint occaecati`, 'stall': `H-10` },
        { 'id': 313, 'title': `Excepturi`, 'stall': `H-9` },
        { 'id': 314, 'title': `Et quas molestias`, 'stall': `H-8` },
        { 'id': 315, 'title': `Corrupti quos dolores`, 'stall': `H-7` },
        { 'id': 316, 'title': `Deleniti atque`, 'stall': `H-6` },
        { 'id': 317, 'title': `Praesentium voluptatum`, 'stall': `H-5` },
      ]
    }
  ],

}
